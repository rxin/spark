...
class CoPartitionedRDD(
    sc: SparkContext,
    var f: (Map[Long, Iterator[InternalRow]]) => Iterator[InternalRow],
    fragments: Seq[QueryFragment],
  ...
  override def compute(s: Partition, context: TaskContext): Iterator[InternalRow] = {
    ...
    val iterators = new collection.mutable.HashMap[Long, Iterator[InternalRow]]
    var i = 0
    while (i < rdds.length) {
      iterators.put(fragments(i).id, rdds(i).iterator(partitions(i), context))
      i += 1
    }
    f(iterators.toMap)
  }
