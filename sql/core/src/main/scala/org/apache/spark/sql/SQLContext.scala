...
class SQLContext(@transient val sparkContext: SparkContext)
  ...
  implicit def createSchemaRDD[A <: Product: TypeTag](rdd: RDD[A]): SchemaRDD = {
    ...
    val schema = catalyst.types.StructType.fromAttributes(attributeSeq)
  ...
  def applySchema(rowRDD: RDD[Row], schema: StructType): SchemaRDD = {
    ...
    val logicalPlan = LogicalRDD(schema.toCatalyst.toAttributes, rowRDD)(self)
  ...
  def jsonRDD(json: RDD[String], schema: StructType): SchemaRDD = {
    ...
    val appliedSchema: catalyst.types.StructType = if (schema ne null) {
      schema.toCatalyst
    } else {
    ...
    applySchema(
      rowRDD, types.util.DataTypeConversions.toPublicType(appliedSchema).asInstanceOf[StructType])
  }
  ...
  def jsonRDD(json: RDD[String], samplingRatio: Double): SchemaRDD = {
    val columnNameOfCorruptJsonRecord = columnNameOfCorruptRecord
    val appliedSchema: catalyst.types.StructType =
      JsonRDD.nullTypeToStringType(
   ...
   */
  private[sql] def parseDataType(dataTypeString: String): catalyst.types.DataType = {
    catalyst.types.DataType.fromJson(dataTypeString)
  }
  ...
  private[sql] def applySchemaToPythonRDD(
    ...
    def needsConversion(dataType: DataType): Boolean = dataType match {
      ...
      case udt: types.UserDefinedType[_] => needsConversion(udt.sqlType)
      case other => false
    }

    val convertedRdd = if (schema.fields.exists(f => needsConversion(f.dataType))) {
      rdd.map(m => m.zip(schema.fields).map {
        case (value, field) => EvaluatePython.fromJava(value, field.dataType.toCatalyst)
      })
    } else {
      rdd
    }
    ...
    new SchemaRDD(this, LogicalRDD(schema.toCatalyst.toAttributes, rowRdd)(self))
  }
}
